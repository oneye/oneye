function smoothScale(val,id,ori) { // oneye
	...
	if(val < ori) {
		if(size > val) {
			...
			setTimeout("smoothScale("+val+",'"+id+"',"+ori+");",100);
		} else {
			enableShadow(id);
		}
	} else {
...
function disableEdShadow(id) { // oneye
	document.getElementById(id+'_shadowleft').parentNode.removeChild(document.getElementById(id+'_shadowleft'));
	document.getElementById(id+'_shadowright').parentNode.removeChild(document.getElementById(id+'_shadowright'));
	document.getElementById(id+'_shadowleftcorner').parentNode.removeChild(document.getElementById(id+'_shadowleftcorner'));
	document.getElementById(id+'_shadowrightcorner').parentNode.removeChild(document.getElementById(id+'_shadowrightcorner'));
	document.getElementById(id+'_shadowtop').parentNode.removeChild(document.getElementById(id+'_shadowtop'));
}

function enableShadow(id) { // oneye
	if(document.getElementById(id+'_shadowleft')) {
		return;
	}
	var obj = document.createElement('div');
	var ifr_father = document.getElementById(id).parentNode.parentNode.parentNode.parentNode;
	...
	obj.setAttribute('id',id+'_shadowleft');
	var fsize = xWidth(ifr.parentNode);
	fsize = fsize - xWidth(ifr);
	fsize = fsize / 2;
	obj.style.left = fsize - 8 + 'px'; // 8 = the size of our element, to not overlap the paper
	ifr_father.appendChild(obj);
	...
	obj.style.top = '109px';
	obj.style.position = 'absolute';
	obj.setAttribute('id',id+'_shadowright');
	var fsize = xWidth(ifr.parentNode);
	fsize = fsize - xWidth(ifr);
	fsize = fsize / 2;
	obj.style.left = fsize + xWidth(ifr) + 'px'; // 8 = the size of our element, to not overlap the paper
	ifr_father.appendChild(obj);
	...
	lcobj.style.top = '95px'; // first 14 = obj.bottom, second 14 = cornerleft_sh.png height
	lcobj.style.left = fsize + xWidth(ifr) - 6 + 'px';

	ifr_father.appendChild(lcobj);

	//top shadow
	var tobj = document.createElement('div');
	tobj.style.width = xWidth(ifr) -12+'px';
	tobj.style.left = fsize+6+'px';
	tobj.style.top = '95px';
...
function disableShadow(id) {} // oneye
...
(function() {
	...
	tinymce.create('tinymce.util.URI', {
		...
		toAbsolute : function(u, nh) { // oneye
			if (u.indexOf(tinymce.baseURL) === 0) {
				return u;
			}
			return tinymce.baseURL + '/' + u;
		},

		toAbsolute_ : function(u, nh) { // oneye
...
(function(tinymce) {
	...
	tinymce.baseURL = new tinymce.util.URI(tinymce.documentBaseURL).toAbsolute_(tinymce.baseURL); // oneye
...
(function(tinymce) {
	...
	tinymce.EditorCommands = function(editor) {
		...
		addCommands({
			...
			SelectZoom : function(command, ui, value) { // oneye
				id = editor.id + '_ifr';
				disableEdShadow(id);
				setTimeout(function () {
					smoothScale(parseInt(6.5 * value), id, xWidth(id));
				}, 10);
			},
